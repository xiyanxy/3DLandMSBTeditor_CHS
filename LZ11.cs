...
*/
            ...
            if (type != 0x11)
            {
                throw new InvalidDataException("提供的流不是有效的LZ-0x11 "
                + "压缩流 (无效类型0x" + type.ToString("X") + ")");
            }
    ...
    public class NotEnoughDataException : IOException {
        ...
        public NotEnoughDataException(long currentOutSize, long totalOutSize)
            : base((totalOutSize < 0 ? "???" : ("0x" + totalOutSize.ToString("X")))
            + "没有足够的可用数据; 0x" + currentOutSize.ToString("X")
            + " 字节写入。") {
    ...
    public class TooMuchInputException : Exception {
        ...
        public TooMuchInputException(long readBytes, long totLength)
            : base("输入包含的数据超出了需要。只用 0x"
             + "0x" + totLength.ToString("X")  + readBytes.ToString("X") + " 字节") {
